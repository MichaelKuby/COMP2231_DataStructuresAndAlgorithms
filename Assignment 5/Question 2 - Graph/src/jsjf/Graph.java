...
public class Graph<T> implements GraphADT<T>
{
    ...
    public void removeVertex(int index)
    {
        ...
        {
            // if the vertex being removed > 0, we conceptually partition the adjacency matrix
            // and migrate the values in each partition as required.
            // shift the values in the upper right and lower left partitions of the adjacency matrix
            if (index > 0)
            ...
            // shift the values in the lower right partition of the adjacency matrix
            for (int col = index; col < adjMatrix.length - 1; col++)
                for (int row = index; row < adjMatrix.length - 1; row++)
                    adjMatrix[col][row] = adjMatrix[col+1][row+1];

            // set last rows to false
            for (int i = 0; i < numVertices; i++)
            {
            ...
            // shift entries in the list of vertices
            for (int col = index; col < numVertices; col++)
                vertices[col] = vertices[col + 1];
    ...
    public boolean isConnected()
    {
        ...
        GraphIterator iter = new GraphIterator(iteratorBFS(0));
